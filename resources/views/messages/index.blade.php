    ...
    <li class="{{ $isUnreadClass }}">
        <a href="{{ route('messages.conversation', ['user' => $otherUser->id, 'articleId' => $latestMessage->article_id ?? 0]) }}">
            <div class="flex items-center">
                <img src="{{ $latestMessage->article ? Storage::url($latestMessage->article->photo) : 'default-image-path.jpg' }}" alt="{{ $latestMessage->article ? $latestMessage->article->title : 'Standardtitel' }}" class="w-8 h-8" />
